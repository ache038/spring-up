...
public class Period {
    ...
    static LocalTime toTime(final String literal) {
        final Optional<DateTimeFormatter> hit = Fn.getJvm(Optional.empty(),
                () -> TIMES.stream().filter(formatter ->
                        null != Fn.getJvm(
                                null,
    ...
    static Date parse(final String literal) {
        return Fn.getJvm(null, () -> {
            ...
            final String pattern = Iso.PATTERNS.get(length);
            if (null != pattern) {
    ...
    static Date parseFull(final String literal) {
        return Fn.getJvm(null, () -> {
            ...
            final Date ret;
            if (null != datetime) {
                ret = Date.from(datetime.atZone(ZoneId.systemDefault()).toInstant());
            } else {
                final LocalDate date = toDate(literal);
                if (null != date) {
                    ret = Date.from(date.atStartOfDay().atZone(ZoneId.systemDefault()).toInstant());
                } else {
                    final LocalTime time = toTime(literal);
                    ret = null == time ? null : parse(time);
                }
            }
            return ret;
        }, literal);
